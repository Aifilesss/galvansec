...
if (bootEl) {
    const lines = [
        ...
        { type: 'cmd', prompt: 'galvan@sec', cmd: ':~$ whoami' },
        { type: 'output', text: 'mario_a_galvan' },
        { type: 'cmd', prompt: 'galvan@sec', cmd: ':~$ neofetch' },
        { type: 'output', text: 'OS: GalvanSec Linux x86_64\nHost: Portfolio\nKernel: 5.15.0-generic\nShell: bash 5.1.16\nResolution: 1920x1080\nTheme: Cyberpunk [GTK3]\nIcons: Papirus [GTK3]\nTerminal: galvan-terminal\nCPU: Intel Core i7 @ 4.2GHz\nMemory: 16384MiB / 32768MiB' },
    ...
    function renderBoot() {
        ...
        if (line.type === 'info') {
            ...
            if (charIndex < text.length) {
                ...
                charIndex++;
                setTimeout(renderBoot, 15 + Math.random() * 25);
            } else {
        ...
        if (line.type === 'output') {
            currentLine = document.createElement('span');
            currentLine.className = 'boot-line boot-output';
            bootEl.appendChild(currentLine);
            const text = line.text;
            if (charIndex < text.length) {
                currentLine.innerHTML = `<span class="boot-output-text">${escapeHtml(text.slice(0, charIndex + 1))}</span>`;
                charIndex++;
                setTimeout(renderBoot, 8 + Math.random() * 15);
            } else {
                currentLine.innerHTML = `<span class="boot-output-text">${escapeHtml(text)}</span>`;
                lineIndex++;
                charIndex = 0;
        ...
        if (line.type === 'cmd') {
            ...
            if (phase === 'prompt') {
                ...
                if (charIndex < hostText.length) {
                    ...
                    charIndex++;
                    setTimeout(renderBoot, 35 + Math.random() * 40);
                } else {
